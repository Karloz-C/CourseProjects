...
#define FORs(i,a,b,step) for(reg i=a;i<b;i+=step)
...
static uint32_t roundKey[32];
__m512i LOW_8BITS;

#define LEFT_SHIFT(RESULT, INPUT, LEFT, TEMP_LEFT)         \
    RESULT = _mm512_slli_epi32(INPUT, LEFT);          \
    TEMP_LEFT = _mm512_srli_epi32(INPUT, 32 - LEFT);      \
    RESULT = _mm512_xor_si512(RESULT, TEMP_LEFT);       \
...
#define INIT_PLAINTEXT(x0, x1, x2, x3, PLAINTEXT, INDEX)         \
    INDEX = _mm512_setr_epi32(0, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48, 52, 56, 60);        \
    x0 = _mm512_i32gather_epi32(INDEX,(int*)(PLAINTEXT + 0), 4);      \
    x1 = _mm512_i32gather_epi32(INDEX,(int*)(PLAINTEXT + 1), 4);      \
    x2 = _mm512_i32gather_epi32(INDEX,(int*)(PLAINTEXT + 2), 4);      \
    x3 = _mm512_i32gather_epi32(INDEX,(int*)(PLAINTEXT + 3), 4);  

#ifdef enable

inline uint32_t ROTL(uint32_t word, int offset) 
{
...
void SIMDSM4Enc(uint32_t* plaintext, uint32_t* RoundKey, uint32_t cipherText[64]) 
{
    __m512i x0, x1, x2, x3, m512RoundKey, NullTemp0, NullTemp1, NullTemp2, NullTemp3, NullTemp4, FOUR_BYTES_XOR;
    ...
    _mm512_store_si512((__m512i*)cipherText, x3);
    _mm512_store_si512((__m512i*)(cipherText + 16), x2);
    _mm512_store_si512((__m512i*)(cipherText + 32), x1);
    _mm512_store_si512((__m512i*)(cipherText + 48), x0);
}
...
uint32_t plaintext[SIZE], ciphertext[SIZE];
double t = 0;
uint32_t key[4] = { 0x01234567,0x89ABCDEF,0xFEDCBA98,0x76543210 };
uint32_t tp[64] = { 0x01234567, 0x89ABCDEF, 0xFEDCBA98, 0x76543210, 0x01234567, 0x89ABCDEF, 0xFEDCBA98, 0x76543210,0x01234567, 0x89ABCDEF, 0xFEDCBA98, 0x76543210, 0x01234567, 0x89ABCDEF, 0xFEDCBA98, 0x76543210,0x01234567, 0x89ABCDEF, 0xFEDCBA98, 0x76543210, 0x01234567, 0x89ABCDEF, 0xFEDCBA98, 0x76543210,0x01234567, 0x89ABCDEF, 0xFEDCBA98, 0x76543210, 0x01234567, 0x89ABCDEF, 0xFEDCBA98, 0x76543210,0x01234567, 0x89ABCDEF, 0xFEDCBA98, 0x76543210, 0x01234567, 0x89ABCDEF, 0xFEDCBA98, 0x76543210,0x01234567, 0x89ABCDEF, 0xFEDCBA98, 0x76543210, 0x01234567, 0x89ABCDEF, 0xFEDCBA98, 0x76543210,0x01234567, 0x89ABCDEF, 0xFEDCBA98, 0x76543210, 0x01234567, 0x89ABCDEF, 0xFEDCBA98, 0x76543210,0x01234567, 0x89ABCDEF, 0xFEDCBA98, 0x76543210, 0x01234567, 0x89ABCDEF, 0xFEDCBA98, 0x76543210 };
uint32_t tc[64];

inline void init()
{
    srand(time(0));
    FOR(i, 0, SIZE)plaintext[i] = rand();
}
...
int main() {
    ...
    LARGE_INTEGER Fre, start, end;
    QueryPerformanceFrequency(&Fre);

    QueryPerformanceCounter(&start);
    InitRoundKey(roundKey, key);

    FOR(i, 0, SIZE >> 6) //32*64 = 16*128 bits
    SIMDSM4Enc(plaintext + (i << 6), roundKey, ciphertext + (i << 6));
    QueryPerformanceCounter(&end);
    printf("runtime:%fs\n", (double)(end.QuadPart - start.QuadPart) / Fre.QuadPart);
}
#endif